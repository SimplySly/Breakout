...
public: 
	XMLLevelLoader();
	~XMLLevelLoader();
	bool LoadFromXML(const std::string& path, LevelInfo &levelInfo, SDL_Renderer* pRenderer, int brickAreaWidth, int brickAreaHeight);
...
private:
	...
	bool LoadBrickList(tinyxml2::XMLElement* levelElement, const XMLLevelContext& levelContext, LevelInfo& levelInfo, int brickAreaWidth, int brickAreaHeight);
